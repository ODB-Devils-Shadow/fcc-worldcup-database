...
echo "$($PSQL "SELECT SUM(winner_goals + opponent_goals) / 2 FROM games")"
...
echo "$($PSQL "SELECT year, name FROM teams INNER JOIN games ON teams.team_id = games.winner_id AND round='Final'")"
